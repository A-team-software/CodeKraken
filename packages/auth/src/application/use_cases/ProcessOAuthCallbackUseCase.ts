import { OAuthStateRepository, OAuthTokenRepository, OAuthTokenAggregate } from "../../domain";
import { Logger, ProviderType } from "@oliver/core";
...
export class ProcessOAuthCallbackUseCase {
    ...
    async execute(cmd: ProcessOAuthCallbackCommand): Promise<ProcessOAuthCallbackResult> {
        ...
        if (providerType === 'board') {
            ...
            if (provider === 'github') {
                const { GitHubService } = await import('@oliver/git');
                tokenResponse = await GitHubService.exchangeCodeForToken(code, redirectUri);
            } else if (provider === 'bitbucket') {
                const { BitbucketService } = await import('@oliver/git');
        ...
        if (providerType === 'board') {
            ...
            const { GitProviderFactory } = await import('@oliver/git');
