...
import { UserJiraSiteAccessProps } from '@oliver/core';

export class UserJiraSiteAccessCollection {
    private static collection: Collection<UserJiraSiteAccessProps>;
    private static initialized = false;

    static async get(): Promise<Collection<UserJiraSiteAccessProps>> {
        if (!this.collection) {
            const db = await MongoConnectionManager.getDb();
            this.collection = db.collection<UserJiraSiteAccessProps>('usersitaccess');
        }
